    ...
    public function ingest(array $payload): array
    {
        ...
        try {
            return DB::transaction(function () use ($parsed, $fingerprint) {
                ...
                if ($existing) {
                    DB::table('cinepoint_daily_snapshots')->where('id', $existing->id)->update(['last_verified_at' => now(), 'updated_at' => now()]);
                    return $this->duplicate(DB::table('cinepoint_daily_snapshots')->find($existing->id));
                }
                $now=now();
                $id=DB::table('cinepoint_daily_snapshots')->insertGetId(['period_date'=>$parsed['period_date'],'payload_fingerprint'=>$fingerprint,'status'=>'running','source_total'=>$parsed['source_total'],'collected_count'=>0,'is_complete'=>false,'started_at'=>$now,'last_verified_at'=>$now,'created_at'=>$now,'updated_at'=>$now]);
