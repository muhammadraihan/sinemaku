...
$(function () {
    ...
    function ticketOptions(selected) {
        ...
        $('#ticket-types-list input.ticket-name').each(function () { var i=$(this).data('index'), name=$(this).val().trim(); if (name) options += '<option value="'+i+'" '+(String(i)===String(selected)?'selected':'')+'>'+escapeHtml(name)+'</option>'; });
        return options;
    }
    ...
    $('#add-capacity').on('click', function(){ if (!$('#ticket-types-list input.ticket-name').length) { showStep(1); return; } addCapacity({}); });
