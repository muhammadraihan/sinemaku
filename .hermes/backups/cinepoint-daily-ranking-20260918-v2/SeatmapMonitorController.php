...
class SeatmapMonitorController extends Controller
{
    ...
    public function ranking(Request $request)
    {
        ...
        $showtimes = $query->get();
        $discoveryMeta = [
            'showtimes_discovered' => $showtimes->count(),
            'films_discovered' => $showtimes->pluck('normalized_film_name')->filter()->unique()->count(),
            'cinemas_discovered' => $showtimes->pluck('normalized_cinema_name')->filter()->unique()->count(),
            'cities_discovered' => $showtimes->pluck('city')->filter()->unique()->count(),
            'chains_discovered' => $showtimes->pluck('chain')->filter()->unique()->values()->all(),
            'last_discovery_at' => optional($showtimes->pluck('last_seen_at')->filter()->max())->toIso8601String(),
        ];
        if ($showtimes->isEmpty()) {
            return response()->json([
                'date' => $date,
                'data' => [],
                'meta' => array_merge($discoveryMeta, [
                    'last_captured_at' => null,
                    'showtimes_with_snapshots' => 0,
                    'collection_state' => 'no_discovery',
        ...
        $ranked = $showtimes->groupBy(function ($showtime) {
            ...
            foreach ($filmShowtimes as $showtime) {
                ...
                if ($latest) {
                    $latestEstimated = ($latestEstimated ?? 0) + (int) ($latest->unavailable_seats ?? $latest->sold_seats ?? 0);
        ...
        })->filter()->values()->sortByDesc('daily_estimated_admissions')->values();
        ...
        return response()->json([
            ...
            'meta' => array_merge($discoveryMeta, [
                ...
                'showtimes_with_snapshots' => $showtimes->filter(fn ($showtime) => $snapshots->has($showtime->uuid))->count(),
                'collection_state' => $ranked->isEmpty() ? 'no_snapshots' : 'fresh',
                'disclaimer' => $this->disclaimer(),
            ]),
        ]);
    }
    ...
    {
        return view('seatmap-monitor.film', ['filmKey' => $film]);
    }
    ...
    public function storeSource(Request $request)
    {
        $data = $request->validate([
            'provider' => ['required', Rule::in(['tix_id', 'cinepolis', 'cgv', 'xxi', 'other'])],
